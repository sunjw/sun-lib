...
size_t SimpleHttp::ProcessData(void* ptr, size_t size, size_t nmemb,
		void* userdata)
{
	std::string str;
	...
	if (size == 1)
	{
		...
		if (m_bUtf8)
			...
			pstrContent->append(strgbktoutf8(str));
	}
...
int SimpleHttp::DoHttp(std::string& strUrl, MapStrStr* pMapData,
		std::string& strRet, bool bUtf8)
{
	std::string strPost;
...
int SimpleHttp::DoPostFile(std::string& strUrl, MapStrStr* pMapData,
		std::string& strFileMapKey, std::string& strRet, bool bUtf8)
{
	...
	while (1)
	{
		...
		{
			// 是文件
			curl_formadd(&pPostBegin, &pPostEnd, CURLFORM_COPYNAME,
					strKey.c_str(), CURLFORM_FILE, (*pMapData)[strKey].c_str(),
					CURLFORM_END);
		}
		else
		{
			...
			curl_formadd(&pPostBegin, &pPostEnd, CURLFORM_COPYNAME,
					strKey.c_str(), CURLFORM_COPYCONTENTS, strValue.c_str(),
					CURLFORM_END);
		}
